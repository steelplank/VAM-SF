...
- **storybrew**, with a created project. Simply open storybrew -> `New project` -> add whatever name and point it to the mapset -> `Start`
...
- Download latest version of VAM:SF from GitHub - it's under `Releases`. Pick the latest version and in `Assets` download the VAM-SF-vx.xx.zip file. **Don't** download Source code if you need this guide please.
...
2. Removes all nc's (new combo) and changes combo colors to white
...
To use that `Quick Publish` script simply go to the `VAM-SF` folder in your project folder, and open `Install.bat` again. Select option **2 - Quick Publish** just like during installation. No errors appear? Cool, everything is ready. You can now upload your map to osu! as you would usually do.

---

## 4. Upgrading to the newest version

New version appeared on GitHub and you want to upgrade? Download the latest version .zip and just replace old `VAM-SF` folder with the new one. Windows will ask you if you want to replace files - simply say yes to all. After that open `Install.bat` and select option **1 - Upgrade / reinstall**. Upgrade is done - as simple as that!

---

## 7. Backups and reverting

Every operation that edits a `.osu` first copies the original into `VAM-SF\backups\<mapset>\`. If you need one file back, copy it out of `backups\` by hand. Or you can open `Install.bat` and select option **7 - Revert .osu to originals**.
